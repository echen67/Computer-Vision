...
def get_tiny_images(image_arrays):
    ...
    feats = [None] * len(image_arrays)
    newSize = 16
    for i in range(len(image_arrays)):
        img = image_arrays[i]
        resized = cv2.resize(img, (newSize, newSize))
...
def nearest_neighbor_classify(train_image_feats, train_labels,
    ...
    for i in range(test_image_feats.shape[0]):
        aSlice = D[:,i]
        minInd = np.argsort(aSlice)
        kminInd = minInd[:k]
        minLabels = [train_labels[i] for i in kminInd]
        c = Counter(minLabels).most_common(1)
        c = c[0][0]
...
def kmeans(feature_vectors, k, max_iter = 100):
    ...
    max_iter = 10   # JUST FOR TESTING

    # Initialize clusters: a list of feature vectors
    clusters = [None]*k             # clusters: put each feature vector into bin based on which centroid it is closest to
    clusters = np.array(clusters)
    
    # Select random feature vectors as centroids
    # centroidsInd = np.random.choice(np.arange(feature_vectors.shape[0]), size=k, replace=False)
    centroidsInd = np.random.choice(np.unique(feature_vectors, axis=0, return_index=True)[1], size=k, replace=False)
    centroids = feature_vectors[centroidsInd, :]

    for m in range(max_iter):
        for i in range(feature_vectors.shape[0]):
            dist = np.sum((centroids - feature_vectors[i,:])**2, axis=1)
            centroidInd = np.argmin(dist)
            if clusters[centroidInd] == None:
                # clusters[centroidInd] = [i]
                clusters[centroidInd] = [feature_vectors[i]]
                # clusters[centroidInd] = feature_vectors[i]
            else:
                # clusters[centroidInd].append(i)
                clusters[centroidInd].append(feature_vectors[i])
                # clusters[centroidInd] = np.append(clusters[centroidInd], feature_vectors[i])

        # Recalculate centroids based on current clusters; One new centroid from each cluster
        # Get the average of all points in that cluster and make that the new centroid
...
def build_vocabulary(image_arrays, vocab_size, stride = 20):
    ...
    for img in image_arrays:
        ...
        xv, yv = np.meshgrid(np.arange(10, img.shape[0]-10, stride), np.arange(10, img.shape[1]-10, stride))
        sift_features = get_siftnet_features(img_final, yv.flatten(), xv.flatten())
        if len(features) == 0:
...
def kmeans_quantize(raw_data_pts, centroids):
    ...
    #     Dslice = D[i,:]                                        # Get one feature vector's distance against all centroids
    #     minInd = np.argmin(Dslice)                             # Get index of the closest centroid
    #     indices.append(minInd)
    # indices = np.array(indices)

    indices = []
    for i in range(raw_data_pts.shape[0]):
        dist = np.sum((centroids - raw_data_pts[i,:])**2, axis=1)
...
def get_bags_of_sifts(image_arrays, vocabulary, step_size = 10):
    ...
    for i in range(len(image_arrays)):
        ...
        img_tensor = torch.from_numpy(img_array)
        img_final = img_tensor.reshape(1, 1, img.shape[0], img.shape[1])
        xv, yv = np.meshgrid(np.arange(10, img.shape[0]-10, step_size), np.arange(10, img.shape[1]-10, step_size))
        sift_features = get_siftnet_features(img_final, yv.flatten(), xv.flatten())  
        indices = kmeans_quantize(sift_features, vocab)                                                             # For each feature, find the centroid it's closest to
