...
def generate_random_stereogram(im_size: Tuple[int, int, int] = (51, 51, 3), disparity: int = 4) -> torch.Tensor:
  ...
  im_left = torch.zeros(im_size, dtype=torch.float)
  # iterate through pixels of image, randomly set to 0 or 1
  # value for each pixel should be same across channels, however
  for h in range(H):
    for w in range(W):
  ...
  # Shift block to the left by disparity
  # h4 = block_size[0]//2
  # w4 = block_size[1]//2
  # block = im_right[w4:W-w4, h4:H-h4, :]
  # print(im_center[0]-block_size[0]//2, im_center[0]+block_size[0]//2+1)
  # print(im_center[1]-block_size[1]//2, im_center[1]+block_size[1]//2+1)
  for h in range(im_center[0]-block_size[0]//2, im_center[0]+block_size[0]//2+1):
    for w in range(im_center[1]-block_size[1]//2, im_center[1]+block_size[1]//2+1):
      for c in range(C):
        im_right[h][w-4][c] = im_right[h][w][c]     # ORIGINAL
        # im_right[h][w][c] = im_right[h][w][c]       # TESTING

  # Fill hole with random values
  for h in range(im_center[0]-block_size[0]//2, im_center[0]+block_size[0]//2+1):
    for w in range(im_center[1]+block_size[1]//2-disparity, im_center[1]+block_size[1]//2+1):
      val = random.randint(0, 1)
      for c in range(C):
        im_right[h][w][c] = val
