...
class HistogramLayer(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        ** You will receive extra-credit if you successfully implement this
        ...
        # b, a = torch.meshgrid(b, a)
        # print(a)
        # print(b)

        test = torch.argmax(cosines, dim=1, keepdim=True)
        mag = torch.norm(im_grads, dim=1)
        occupancy = torch.zeros_like(cosines)

        # FOR LOOPS
        height = occupancy.shape[2]
        width = occupancy.shape[3]
        for h in range(height):
            for w in range(width):
                binInd = test[0][0][h][w]
                occupancy[0][binInd][h][w] = mag[0][h][w]

        # TESTING
        # binInd = torch.flatten(test[0, 0, :, :]).float()
        # mag = torch.flatten(mag[0, :, :])
        # print(binInd.dtype)
        # print(mag.dtype)
        # a, b = torch.meshgrid(binInd, mag)
        # print(a)
        # print(b)
        ...
        # binInd = test[0, 0, :, :]
        # print(binInd)
        # height = torch.arange(occupancy.shape[2])
        # width = torch.arange(occupancy.shape[3])
        # occupancy[0, binInd, :, :] = mag[0, :, :]
        # occupancy[0, binInd, height, width] = mag[0, height, width]

        # occupancy[0, binInd, :, :] = 100
        # print("occupancy", occupancy[:, binInd, :, :].shape)
...
class SubGridAccumulationLayer(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.layer = nn.Conv2d(in_channels=8, out_channels=8, padding=2, kernel_size=4, groups=8, bias=False)
        # self.layer.weight = nn.Parameter(torch.ones(8, 1, 4, 4))
        self.layer.weight = nn.Parameter(torch.ones(8, 1, 2, 2))
...
def angles_to_vectors_2d_pytorch(angles: torch.Tensor) -> torch.Tensor:
    ...
    top = torch.cos(angles)
    bottom = torch.sin(angles)
    angle_vectors = torch.stack((top, bottom), dim=1)
...
class SIFTOrientationLayer(nn.Module):
    ...
    def get_orientation_bin_weights(self) -> torch.nn.Parameter():
        ...
        const = np.pi/8
        # const = 0
        unit_angles = np.array([0+const, (np.pi/4)+const, (np.pi/2)+const, (0.75*np.pi)+const, (np.pi)+const, ((5/4)*np.pi)+const, 1.5*np.pi+const, (7/4)*np.pi+const])
        # unit_angles = np.array([0, (np.pi/4), (np.pi/2), (0.75*np.pi), (np.pi), ((5/4)*np.pi), 1.5*np.pi, (7/4)*np.pi])
        unit_vectors = angles_to_vectors_2d_pytorch(torch.from_numpy(unit_angles))
        unit_vectors = unit_vectors.float()
        ones = torch.ones(2,2)
        ones[1][0] = 0
        ones[0][1] = 0
        stacked = torch.cat((unit_vectors, ones))
        final = torch.reshape(stacked, (10, 2, 1, 1))
        weight_param = nn.Parameter(final)
...
class SIFTNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.net = nn.Sequential(image_gradients_layer, sift_orientation_layer, histogram_layer, subgrid_accumulation_layer)
...
def get_siftnet_features(img_bw: torch.Tensor, x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    siftnet = net(img_bw)   # shape (1, 8, 17, 17)
    fvs = []

    for i in range(x.shape[0]):
        x_grid, y_grid = get_sift_subgrid_coords(x[i], y[i])

        temp = siftnet[0, :, y_grid, x_grid]

        flat = temp.flatten()
        flatsum = torch.sum(flat*flat)
        flatsum = torch.sqrt(flatsum)
        flat = torch.div(flat, flatsum)
        flat = torch.pow(flat, 0.9)
