...
class HarrisNet(nn.Module):
    ...
    def __init__(self):
        ...
        nms_layer = NMSLayer()
        self.net = nn.Sequential(image_gradients_layer, channel_product_layer, second_moment_matrix_layer, corner_response_layer, nms_layer)
...
class SecondMomentMatrixLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gaussian = get_gaussian_kernel(self.ksize, self.sigma)
        gaussian = torch.reshape(gaussian, (1, 1, gaussian.shape[0], gaussian.shape[1]))
        gaussian = torch.cat((gaussian, gaussian, gaussian))
        padX = gaussian.shape[2]//2
        padY = gaussian.shape[3]//2
        output = nn.functional.conv2d(x, gaussian, padding=(padX, padY), groups=3)
...
class CornerResponseLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        chunks = torch.chunk(x, 3, 1)
        Ixx = chunks[0]
        Iyy = chunks[1]
        Ixy = chunks[2]

        det = torch.mul(Ixx, Iyy) - torch.mul(Ixy, Ixy)

        trace = Ixx + Iyy
        trace = torch.mul(trace, trace)
        alpha = 0.05
        trace = torch.mul(trace, alpha)
...
class NMSLayer(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        thresholded = torch.where(x > med, x, y)

        maxpool = nn.MaxPool2d(7, stride=1, padding=3, return_indices=True)
        maxed, indices = maxpool(thresholded)

        z = torch.ones(x.shape)
        binarized = torch.where(x >= maxed, z, y)

        output = torch.mul(x, binarized)
...
def get_interest_points(image: torch.Tensor, num_points: int = 4500) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    reshaped = torch.reshape(R, (-1,))

    if num_points > reshaped.shape[0]:
        num_points = reshaped.shape[0]

    val, ind = torch.topk(reshaped, num_points)
    threshold = val[val.shape[0]-1]

    y = torch.zeros(R.shape)
    masked = torch.where(R >= threshold, R, y)

    nonzero = torch.nonzero(masked)
    ...
    for i in range(nonzero.shape[0]):
        x.append(nonzero[i][3])
        y.append(nonzero[i][2])
        confidences.append(R[0][0][nonzero[i][2]][nonzero[i][3]])
    ...
    newx = x.clone()
    newy = y.clone()
    confidences, indices = torch.sort(confidences, descending=True)
...
def remove_border_vals(img, x: torch.Tensor, y: torch.Tensor, c: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    xmask1 = torch.ge(x, 8)
    xmask2 = torch.le(x, img.shape[3]-8)
    xmask3 = xmask1 * xmask2

    x = torch.masked_select(x, xmask3)
    y = torch.masked_select(y, xmask3)
    c = torch.masked_select(c, xmask3)

    ymask1 = torch.ge(y, 8)
    ymask2 = torch.le(y, img.shape[2]-8)
    ymask3 =  ymask1 * ymask2

    x = torch.masked_select(x, ymask3)
    y = torch.masked_select(y, ymask3)
    c = torch.masked_select(c, ymask3)
