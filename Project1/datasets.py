...
def make_dataset(path: str) -> Tuple[List[str],List[str]]:
  ...
  for entry in os.listdir(path):
    if entry[1] == "a" or entry[2] == "a":
      images_a.append(path + "/" + entry)
    else:
      images_b.append(path + "/" + entry)
...
def get_cutoff_frequencies(path: str) -> List[int]:
  ...
  cutoff_frequencies = np.empty(0)
  alist = []
  file = open(path, "r")
  for line in file:
    line = line.strip('\n')
    line = int(line)
    np.append(cutoff_frequencies, [7])
    alist.append(line)
  file.close()

  cutoff_frequencies = np.asarray(alist)
  # cutoff_frequencies = np.empty(1)
...
class HybridImageDataset(data.Dataset):
  ...
  def __getitem__(self, idx: int) -> Tuple[torch.Tensor,torch.Tensor,int]:
    ...
    pathA = self.images_a[idx]
    pathB = self.images_b[idx]
    imA = Image.open(pathA)
    imB = Image.open(pathB)
    imA.transpose(PIL.Image.TRANSPOSE)
    imB.transpose(PIL.Image.TRANSPOSE)
    resultA = self.transform(imA)
    resultB = self.transform(imB)
    imA.close()
    imB.close()
